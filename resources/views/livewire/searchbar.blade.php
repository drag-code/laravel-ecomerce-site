<div class="flex-1 relative" x-data>
    <form action="{{ route('search') }}" autocomplete="off">
        <x-jet-input name="search" wire:model="search" type="text" class="w-full" placeholder="¿Qué estas buscando?" />
        <button type="submit"
            ...
            <div class="px-4 py-3">
                @forelse($products as $product)
                    <a class="flex gap-2 mb-2 cursor-pointer" href="{{ route('products.show', $product) }}">
                        <img class="object-cover object-center h-12 w-16"
                            src="{{ Storage::url($product->image->first()->url) }}" alt="">
